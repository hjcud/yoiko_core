...
공식전은 서버 설정 시간대를 기준으로 매일 20시에 열립니다. 서버는 19시부터 경기장을 준비하고 참가 등록, 출전 명단 확정과 베팅 마감을 차례로 처리한 뒤 예약 시각에 경기를 시작합니다. 실제 운영에서는 예약 경주와 경기장 상태가 겹치는 수동 준비·개최 명령을 사용하지 않습니다.
...
공식전 결과로 레이스 포인트를 얻어 D부터 S까지 진급합니다. 타임어택에서는 성장 보상 없이 다양한 난이도와 코스를 시험할 수 있으며, 관전자는 전용 HUD와 경기 해설을 통해 순위, 지구력, 스킬 발동과 주요 구간을 확인합니다. 베팅, 경기 재생과 운영자용 코스·효과 미리보기도 같은 경기 상태를 사용합니다.
